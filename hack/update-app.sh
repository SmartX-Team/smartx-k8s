...
for manifest_path in $(
    ...
    if [ "x$(cat "${manifest_path}" | yq '.spec.source == null')" == 'xtrue' ]; then
        continue
    fi

    # Read the source chart name
    app_name="$(cat "${manifest_path}" | yq '.metadata.name')"
    ...
    # Read the source chart name
    source_chart="$(cat "${manifest_path}" | yq '.spec.source.chart')"
    ...
    source_repo_url="$(cat "${manifest_path}" | yq '.spec.source.repoUrl')"
    ...
    source_version="$(cat "${manifest_path}" | yq '.spec.source.version')"
    ...
    if [ "x${source_is_oci}" == 'xtrue' ]; then
        latest_version="$(helm show chart "oci://${source_repo_url}/${source_chart}" | yq '.version')" || continue
    else
        latest_version="$(helm show chart --repo "${source_repo_url}" "${source_chart}" | yq '.version')" || continue
    fi
